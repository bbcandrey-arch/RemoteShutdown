...
public sealed class SessionRelayClient : IDisposable
{
    ...
    /// <summary>
    /// НЕ ждёт завершения фонового цикла синхронно — раньше здесь был
    /// _loop.Wait(2 секунды) на UI-потоке, и "Выход" из трея блокировал его на эти же
    /// 2 секунды с ЕЩЁ ДЕРЖИМЫМ мьютексом единственного экземпляра (см.
    /// SingleInstanceGuard): если пользователь в этом окне успевал кликнуть по ярлыку
    /// ещё раз, новый запуск просто сигналил умирающему старому процессу и сам сразу
    /// завершался — со стороны выглядело как "второй раз приложение не запускается".
    /// Просто просим цикл остановиться (Cancel) и не ждём — процесс всё равно
    /// завершается следом (Application.Exit()), ОС сама подчистит фоновый поток.
    /// </summary>
    public void Dispose()
    {
        _cts.Cancel();
