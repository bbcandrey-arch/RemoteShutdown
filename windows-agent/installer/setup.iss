...
; Права на папку данных: она внутри {commonappdata}, где у обычных пользователей по
; умолчанию нет записи — Tray (обычный пользователь) падает с UnauthorizedAccessException
; на pairing-qr.png/agent.db, если ставил агент кто-то другой (например, администратор), а
; пользуется треем другая учётка. Сама служба это тоже чинит на каждом старте (см.
; AgentDataFolderAcl.cs) — этот шаг лишний только на машинах с уже актуальной службой,
; но не мешает и подчищает права сразу же, не дожидаясь первого перезапуска службы.
; *S-1-5-11 — статический SID "Прошедшие проверку" (Authenticated Users), не зависит от
; локализации Windows, в отличие от текстового имени группы.
Filename: "{sys}\icacls.exe"; Parameters: """{app}"" /grant *S-1-5-11:(OI)(CI)M /T"; Flags: runhidden; StatusMsg: "Настройка прав доступа к данным агента..."
